...
export const transactionHistoryInputSchema: z.ZodType<{ limit?: number }>;
export const operationCatalog: Readonly<Record<string, { name: string; description: string; path: string; inputSchema: z.ZodTypeAny; outputSchema: z.ZodTypeAny }>>;
...
export function executeMcpReadOperation(operation: string, input: unknown, request: (path: string, operation: string) => Promise<{ ok: boolean; data?: unknown; operationError?: OperationError }>): Promise<{ content: Array<{ type: "text"; text: string }>; isError?: true }>;
export function executeCliReadOperation(operation: string, input: unknown, request: (path: string, operation: string) => Promise<{ ok: boolean; data?: unknown; operationError?: OperationError }>): Promise<{ success: true; data: unknown }>;
