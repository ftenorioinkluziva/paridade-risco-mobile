...
const canonicalUnauthorized = {
  success: false,
  error: { code: "UNAUTHORIZED", category: "authorization", message: "Missing, invalid or expired session token", retryable: false },
};
...
for (const scenario of ["missing", "invalid", "expired"]) {
  test(`${scenario} session returns canonical 401`, async () => {
...
test("valid session reaches MCP using the Bearer token and never x-user-id", async () => {
  const observed = [];
