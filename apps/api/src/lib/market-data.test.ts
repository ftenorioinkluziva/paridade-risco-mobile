...
test("mantém exatamente a cesta estratégica de ETFs", () => {
  ...
  assert.equal(STRATEGIC_ETF_TICKERS.length, 11);
});
...
test("abre renda fixa às 10:00 e renda variável às 10:05", () => {
  ...
  assert.equal(fullWindow.length, 11);
  assert.ok(fullWindow.includes("BOVA11"));
});

test("estima 12.826 chamadas mensais com intervalo de 8 minutos", () => {
  assert.equal(monthlyCallEstimate(), 12_826);
});
